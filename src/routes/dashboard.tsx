...
type EventRow = {
  ...
  action: string; event_type?: string; violation: boolean; control_ref?: string;
};
type AlertRow = {
  timestamp: string; rule: string; rule_name?: string; severity: "CRITICAL" | "HIGH" | "MEDIUM" | "LOW";
...
function DashboardComponent() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      if (e?.status === 401) {
        setErrBanner("Session expired — please log in again");
      } else {
  ...
  const alertsFiltered = useMemo(() => {
    ...
    return alerts.filter((a) =>
      ...
      (!q || (a.rule_name || a.rule || "").toLowerCase().includes(q) || a.hostname.toLowerCase().includes(q) || a.agent_id.toLowerCase().includes(q))
    );
  }, [alerts, alertFilters]);
      ...
      <header className="gx-topbar">
        ...
        <div className="gx-topbar-right">
          ...
          <div className="gx-status-pill">
            ...
            <span style={{ color: connStatus === "live" ? "var(--gx-green)" : connStatus === "demo" ? "var(--gx-yellow)" : "var(--gx-muted)" }}>
      ...
      <main className="gx-main">
        ...
        <div className="gx-stat-row">
          <StatCard color="cyan" label="Total Events" value={summary?.totals.events}
            sub={summary ? `${summary.totals.agents} agents reporting` : "connect to load"} trend={trend("events")} />
        ...
        <div className="gx-grid2">
          <Panel
            title={
              <span>Violations
                <select value={rangeSel} onChange={(e) => setRangeSel(e.target.value as any)}
                  style={{ background: "var(--gx-input-bg)", color: "var(--gx-text)", border: "1px solid var(--gx-border2)", padding: "2px 6px", fontFamily: "var(--gx-font)", fontSize: 11, marginLeft: 6 }}>
            ...
            right={<><span>{rangeData.length ? `${rangeData.reduce((s, r) => s + r.count, 0)} total` : "--"}</span><ChartTypeSelect value={violationsChart} onChange={setViolationsChart} /></>}
          >
            <div className="gx-chart-wrap">
              <FlexChart type={violationsChart} data={rangeData.map((r) => ({ name: r.hour, value: r.count }))}
                colorFor={() => "#ff3355"} baseColor="#ff3355" axisColor={axisColor} gridColor={gridColor} tooltipStyle={tooltipStyle} />
            </div>
          </Panel>
          <Panel
            title="Events by module"
            right={<><span>{summary ? `${summary.events_by_module.length} modules • ${summary.totals.events.toLocaleString()} total events` : "--"}</span><ChartTypeSelect value={modulesChart} onChange={setModulesChart} /></>}
          >
            <div className="gx-chart-wrap">
              <FlexChart type={modulesChart} data={(summary?.events_by_module || []).map((m) => ({ name: m.module, value: m.count }))}
                colorFor={(n) => MOD_COLORS[n] || "#00c8ff"} baseColor="#00c8ff" axisColor={axisColor} gridColor={gridColor} tooltipStyle={tooltipStyle} />
            </div>
          </Panel>
        </div>
        ...
        <div className="gx-grid3">
          <Panel title="Alert Severity"
            right={<><span>{summary ? `${summary.totals.alerts.toLocaleString()} total alerts` : "--"}</span><ChartTypeSelect value={severityChart} onChange={setSeverityChart} /></>}>
            <div className="gx-chart-wrap">
              <FlexChart type={severityChart} data={(summary?.alerts_by_severity || []).map((s) => ({ name: s.severity, value: s.count }))}
                colorFor={(n) => SEV_COLORS[n] || "#4a6070"} baseColor="#ff3355" axisColor={axisColor} gridColor={gridColor} tooltipStyle={tooltipStyle} />
            </div>
          </Panel>
          <Panel title="Top Agents by Events" right={<><span>top 6</span><ChartTypeSelect value={topAgentsChart} onChange={setTopAgentsChart} /></>}>
            <div className="gx-chart-wrap">
              <FlexChart type={topAgentsChart} data={(summary?.top_alerting_agents || []).slice(0, 6).map((a) => ({ name: a.hostname, value: a.violation_count }))}
                colorFor={() => "#00c8ff"} baseColor="#00c8ff" horizontal axisColor={axisColor} gridColor={gridColor} tooltipStyle={tooltipStyle} />
            </div>
          </Panel>
          <Panel title="Events Timeline - 30 Days" right={<><span>events vs violations</span><ChartTypeSelect value={timelineChart} onChange={setTimelineChart} /></>}>
            <div className="gx-chart-wrap">
              <FlexTimeline type={timelineChart} data={timeline} axisColor={axisColor} gridColor={gridColor} tooltipStyle={tooltipStyle} />
            </div>
          </Panel>
        </div>
            ...
            <>
              ...
              <div style={{ overflowX: "auto" }}>
                <table className="gx-tbl">
                  ...
                  <tbody>
                    {eventsLoading
                      ? <tr><td colSpan={7}><div className="gx-loading">LOADING...</div></td></tr>
                      : events.length === 0
                        ? <tr><td colSpan={7}><div className="gx-empty">Connect to manager to see live data</div></td></tr>
                        : events.map((r, i) => (
                          <tr key={i}>
              ...
              <div className="gx-pager">
                <span className="info">
                  {eventsTotal === 0 ? "0 events" : `${formatNum(evOffset + 1)}–${formatNum(Math.min(evOffset + evLimit, eventsTotal))} of ${formatNum(eventsTotal)} events`}
                </span>
                {(() => {
            ...
            <>
              ...
              <div style={{ overflowX: "auto" }}>
                <table className="gx-tbl">
                  ...
                  <tbody>
                    {alertsPaged.length === 0
                      ? <tr><td colSpan={5}><div className="gx-empty">No alerts match filters</div></td></tr>
                      : alertsPaged.map((a, i) => (
                        <tr key={i}>
          ...
          {tab === "agents" && (
            <>
              ...
              {agentsFiltered.length === 0
                ? <div className="gx-empty">{agents.length === 0 ? "Connect to manager to see live data" : "No agents match filters"}</div>
                : agentsPaged.map((a) => {
  ...
  return (
    <div className={`gx-stat-card c-${color}`}>
      ...
      <div className={`gx-stat-trend ${trend.dir || ""}`}>
        {trend.dir === "up" ? `▲ +${trend.diff} since last sync` : trend.dir === "down" ? `▼ -${trend.diff} since last sync` : ""}
      </div>
    </div>
  );
}
...
function ChartTypeSelect({ value, onChange }: { value: ChartType; onChange: (v: ChartType) => void }) {
  return (
    <select value={value} onChange={(e) => onChange(e.target.value as ChartType)} title="Chart type"
      style={{ background: "var(--gx-input-bg)", color: "var(--gx-text)", border: "1px solid var(--gx-border2)",
        padding: "2px 6px", fontFamily: "var(--gx-font)", fontSize: 10, marginLeft: 8,
        textTransform: "uppercase", letterSpacing: "0.1em", cursor: "pointer" }}>
      {CHART_TYPES.map((t) => <option key={t} value={t}>{t.toUpperCase()}</option>)}
    </select>
  );
}

function HeatmapGrid({ data, colorFor }: { data: { name: string; value: number }[]; colorFor: (n: string) => string }) {
  const max = Math.max(1, ...data.map((d) => d.value));
  const cols = Math.min(Math.max(2, Math.ceil(Math.sqrt(data.length))), 8);
  return (
    <div style={{ display: "grid", gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: 4, padding: 8, height: "100%", overflow: "auto" }}>
      {data.map((d) => {
        const intensity = d.value / max;
        return (
          <div key={d.name} title={`${d.name}: ${d.value}`}
            style={{ background: colorFor(d.name), opacity: 0.25 + intensity * 0.75, borderRadius: 4, padding: "6px 8px",
...
function FlexChart({ type, data, colorFor, baseColor, horizontal, axisColor, gridColor, tooltipStyle }: {
  type: ChartType; data: { name: string; value: number }[]; colorFor: (n: string) => string;
  baseColor: string; horizontal?: boolean; axisColor: string; gridColor: string; tooltipStyle: React.CSSProperties;
}) {
  ...
  if (type === "scatter") {
    ...
    return (
      <ResponsiveContainer><ScatterChart margin={{ top: 5, right: 8, left: -20, bottom: 0 }}>
        ...
        <ZAxis range={[60, 60]} />
        <Tooltip contentStyle={tooltipStyle} cursor={{ strokeDasharray: "3 3" }} formatter={(v: any, _n: any, p: any) => [v, p?.payload?.name]} />
...
function FlexTimeline({ type, data, axisColor, gridColor, tooltipStyle }: {
  type: ChartType; data: { date: string; events: number; violations: number }[];
          ...
          {data.map((d) => {
            ...
            return (
              <div key={d.date} title={`${d.date} • events ${d.events} • viol ${d.violations}`}
                style={{ background: "#00c8ff", opacity: 0.18 + t * 0.82, borderRadius: 3, minHeight: 38,
                  display: "flex", flexDirection: "column", justifyContent: "space-between",
                  padding: "4px 5px", fontFamily: "Share Tech Mono, monospace", fontSize: 9, color: "#001018" }}>
                <span>{d.date}</span>
